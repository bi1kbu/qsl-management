...
业余无线电 QSL 卡片管理系统是一个面向 Halo 2 的 QSL 业务插件，用于把通联记录、实体 QSL 卡申请、卡片创建、制卡签发、发信确认、送达确认、收卡编号、公开查询、线上换卡、线下换卡、邮件通知、数据备份和本地打印桥接整合到同一套可持久化流程中。
...
- 通联业务：从 QSO/SWL 记录出发，完成实体卡申请审核、创建卡片、制卡、发信和送达确认。
...
- 后台可审核通过或拒绝申请；线上换卡审核通过后由审核页显式创建卡片。
...
- 提供唯一公开入口 `/qsl_card`，用户按呼号查询 QSO、复选通联记录，并为每条记录选择卡片版本。
- QSO 查询实时展示可申请、申请未通过可重新申请、审核中、建卡中或异常、待制卡、已制卡、已打包、已发卡和已签收状态；不可申请记录仍显示但禁用选择。
- 个人地址每次重新填写，页面不读取历史个人地址；卡片局使用公开清单，新增卡片局通过本台邮箱联系。
- 后台“通联业务 → 实体卡申请审核”通过申请后立即创建全部卡片，并保存 QSO 与卡片编号的一对一映射；部分失败可幂等重试。
...
- 数据基于 Halo Extension 持久化，覆盖通联记录、卡片记录、实体卡申请及 QSO 占用、换卡申请、线下活动、地址簿、卡片局、设备库、系统参数、审计日志和导入导出任务。
...
- 通联业务：维护通联日志、创建卡片、实体卡申请审核、制卡签发、发信确认和送达确认。
