...
- 页面结构：
  ...
  - 封面打印页：字段映射使用程序内置规则（不可编辑），显示所有未打包（`envelopePrinted=false`）的卡片封面记录，基于已拉取源拼接队列、预览、打印（不自动回写）。
  ...
  - 队列过滤规则（内置）：卡片打印仅纳入“未制卡（`cardIssued=false`）”记录；封面打印纳入所有“未打包（`envelopePrinted=false`）”卡片记录；补打信封不按卡片状态过滤。
