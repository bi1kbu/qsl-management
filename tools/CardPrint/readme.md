...
- 页面结构：
  ...
  - 排版方式：`layout_mode（排版方式）` 支持 `horizontal（横向）`、`vertical（纵向）`、`mixed_vertical（混合纵向）`，默认横向。纵向将全部字形顺时针旋转 `90°` 并从上向下排列；混合纵向会旋转 ASCII 英文字母、数字、英文冒号 `:` 和半角空格，中文、全角冒号 `：` 及其他字符保持正向并逐字排成一列。旋转后的半角内容按原字形宽度计算紧凑间距，不再使用完整行高。第一列位于字段区域最右侧，内容超过打印高度时自动向左换列，超过打印宽度后截断。
