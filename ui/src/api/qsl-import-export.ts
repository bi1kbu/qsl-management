...
const datasetConfigs: DatasetConfig[] = [
  ...
  },
  {
    ...
    toRow: (item) => ({
      ...
      cardCreated: String(Boolean(item.status?.cardCreated)),
      cardCreatedAt: String(item.status?.cardCreatedAt ?? ''),
      cardCreatedBy: String(item.status?.cardCreatedBy ?? ''),
      createdCardRecordName: String(item.status?.createdCardRecordName ?? ''),
    }),
    fromRow: (row) => ({
      spec: {
        sceneType: row.sceneType ?? 'QSO',
      ...
      status: {
        ...
        cardCreated: parseBoolean(row.cardCreated ?? ''),
  ...
  },
  {
    ...
    toRow: (item) => ({
      ...
      telephone: String(item.spec?.telephone ?? ''),
      postalCode: String(item.spec?.postalCode ?? ''),
      destinationCountry: String(item.spec?.destinationCountry ?? ''),
      address: String(item.spec?.address ?? ''),
      email: String(item.spec?.email ?? ''),
  ...
  },
  {
    ...
    toRow: (item) => ({
      ...
      qsoCardCreatedMailPolicy: String(item.spec?.qsoCardCreatedMailPolicy ?? ''),
      qsoCardSentMailPolicy: String(item.spec?.qsoCardSentMailPolicy ?? ''),
      qsoCardReceivedMailPolicy: String(item.spec?.qsoCardReceivedMailPolicy ?? ''),
      onlineCardCreatedMailPolicy: String(item.spec?.onlineCardCreatedMailPolicy ?? ''),
      onlineCardSentMailPolicy: String(item.spec?.onlineCardSentMailPolicy ?? ''),
      onlineCardReceivedMailPolicy: String(item.spec?.onlineCardReceivedMailPolicy ?? ''),
      onlineExchangeReviewedMailPolicy: String(item.spec?.onlineExchangeReviewedMailPolicy ?? ''),
      ...
      aiMaxConcurrentRequests: String(item.spec?.aiMaxConcurrentRequests ?? ''),
      ...
      aiCallbookAddressPrompt: String(item.spec?.aiCallbookAddressPrompt ?? ''),
      qrzComEnabled: String(Boolean(item.spec?.qrzComEnabled)),
      qrzComUsername: String(item.spec?.qrzComUsername ?? ''),
      qrzComSecretName: String(item.spec?.qrzComSecretName ?? ''),
      qrzComXmlBaseUrl: String(item.spec?.qrzComXmlBaseUrl ?? ''),
      qrzCnEnabled: String(Boolean(item.spec?.qrzCnEnabled)),
      qrzCnUsername: String(item.spec?.qrzCnUsername ?? ''),
      qrzCnSecretName: String(item.spec?.qrzCnSecretName ?? ''),
      qrzCnLookupUrlTemplate: String(item.spec?.qrzCnLookupUrlTemplate ?? ''),
      qrzTimeoutSeconds: String(item.spec?.qrzTimeoutSeconds ?? ''),
    ...
    fromRow: (row) => ({
      spec: {
        ...
        qsoCardCreatedMailPolicy: row.qsoCardCreatedMailPolicy ?? '',
        qsoCardSentMailPolicy: row.qsoCardSentMailPolicy ?? '',
        qsoCardReceivedMailPolicy: row.qsoCardReceivedMailPolicy ?? '',
        onlineCardCreatedMailPolicy: row.onlineCardCreatedMailPolicy ?? '',
        onlineCardSentMailPolicy: row.onlineCardSentMailPolicy ?? '',
        onlineCardReceivedMailPolicy: row.onlineCardReceivedMailPolicy ?? '',
        onlineExchangeReviewedMailPolicy: row.onlineExchangeReviewedMailPolicy ?? '',
        ...
        aiMaxConcurrentRequests: parseInteger(row.aiMaxConcurrentRequests ?? ''),
        ...
        aiCallbookAddressPrompt: row.aiCallbookAddressPrompt ?? '',
        qrzComEnabled: parseBoolean(row.qrzComEnabled ?? ''),
        qrzComUsername: row.qrzComUsername ?? '',
        qrzComSecretName: row.qrzComSecretName ?? '',
        qrzComXmlBaseUrl: row.qrzComXmlBaseUrl ?? '',
        qrzCnEnabled: parseBoolean(row.qrzCnEnabled ?? ''),
        qrzCnUsername: row.qrzCnUsername ?? '',
        qrzCnSecretName: row.qrzCnSecretName ?? '',
        qrzCnLookupUrlTemplate: row.qrzCnLookupUrlTemplate ?? '',
        qrzTimeoutSeconds: parseInteger(row.qrzTimeoutSeconds ?? ''),
      },
      status: {
