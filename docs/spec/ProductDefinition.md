...
2. `onlineExchangeRequestPolicy`：线上换卡页面收到表单的处理策略，取值为 `DISABLED`、`MANUAL`、`AUTO_APPROVE`，分别对应关闭线上换卡、手动审核、自动通过。`onlineAutoApprovedRequestMailPolicy` 控制系统自动审批通过后是否向 `StationProfile.spec.myEmail` 发送本台通知，取值为 `AUTO_SKIP` 或 `AUTO_SEND`。`requiresExchangeReview` 作为历史兼容字段保留，未配置新字段时 `false` 映射为自动通过，其他情况映射为手动审核。
...
2. 创建卡片必须关联 QSO/SWL 记录，候选列表排除已写入 `CardRecord.spec.qsoRecordName` 的记录；若选择“不创建卡片”，写入不占用 `C{序号}` 的占位 `CardRecord`，该记录后续不再出现在候选列表中。通联业务创建卡片页下方展示“待创建记录”，数据来源为尚未创建卡片且未标记“不创建卡片”的 QSO/SWL 记录；操作栏提供“创建卡片”按钮，一键生成正式 `CardRecord`，默认卡片版本为系统内置“不发卡”，业务备注为空。通联业务创建卡片默认 `CardRecord.spec.cardRemarks` 为“通联愉快，期待空中常见。\nNice QSO，Hope to catch you on the air often.”。
...
3. 同一呼号存在 `ONLINE_EYEBALL` 待审核换卡申请时，公开提交接口拒绝再次提交；待后台审核通过或审核拒绝后，才允许该呼号再次提交。当 `SystemSetting.spec.onlineExchangeRequestPolicy=AUTO_APPROVE` 时，公开提交成功后立即执行系统自动审批，审核说明固定为“系统自动审批通过”；自动审批只更新申请状态，不自动创建卡片。若 `onlineAutoApprovedRequestMailPolicy=AUTO_SEND`，自动审批通过后向本台通信地址中的电子邮件发送本台通知。当策略为 `DISABLED` 时，公开提交接口拒绝接收新申请。
