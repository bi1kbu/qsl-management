...
14. 打印预设字段支持 `layout_mode（排版方式）`：`horizontal（横向）`、`vertical（纵向）`、`mixed_vertical（混合纵向）`，默认横向。纵向将全部字形顺时针旋转 `90°`，混合纵向仅旋转 ASCII 英文字母和数字，其他字符保持正向并从上向下逐字排列；旋转后的半角字母和数字按原字形宽度采用紧凑间距。
