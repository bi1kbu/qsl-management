...
6. AI 配置字段：``。AI API Key 使用 Halo `Secret` 存储，系统参数只保存 Secret 引用和非敏感配置；AI 功能默认关闭。`aiMaxConcurrentRequests` 控制服务端同时调用第三方 AI 的请求数，默认 1，最大 10；超过并发上限的请求在服务端排队，429 限流错误会短延迟重试。配置页展示完整提示词模板，支持编辑并按项重置为内置默认值。自定义提示词必须保持系统指定的 JSON 返回结构：线上换卡导入顶层字段为 `rows`，地址整理顶层字段为 `items`，呼号地址获取返回 `callSign/recipientName/telephone/postalCode/address/email/confidence/message`，不得修改字段名、字段类型和状态枚举值；线上换卡导入提示词需保留 `{defaultCardVersion}`、`{mode}`、`{text}` 占位符，地址整理提示词需保留 `{rows}` 占位符，呼号地址获取提示词需保留 `{provider}`、`{callSign}`、`{features}` 占位符。线上换卡导入 AI 解析、地址管理 AI 地址整理和 QRZ 地址预览均必须在服务端鉴权后调用，AI 结果只作为预览或建议，不绕过现有导入和地址写入校验。
7. QRZ 地址获取配置字段：`qrzComEnabled/qrzComUsername/qrzComSecretName/qrzComXmlBaseUrl/qrzCnEnabled/qrzCnUsername/qrzCnSecretName/qrzCnLookupUrlTemplate/qrzTimeoutSeconds`。QRZ.COM 采用官方 XML 接口；QRZ.CN 采用可配置查询 URL 模板，但服务端只允许访问 QRZ.COM / QRZ.CN 官方域名，禁止将配置项作为任意服务端请求入口。QRZ 凭据测试需先用临时配置完成远程校验，通过后才写入系统参数与 Secret。服务端优先用用户名和密码登录获取 Cookie，再用 Cookie 抓取查询页面，保留手工 Cookie 作为备用登录态。服务端识别 QRZ.CN `/call/` 查询页面时，先按配置 URL 原样请求，再按站内表单提交呼号，兼容 `q` 与 `callsign` 两类字段；由于 QRZ.CN 旧站存在概率性返回呼号搜索页的问题，每次请求结果都必须校验是否包含目标呼号，有效结果才允许交给 AI 解析，否则自动重试并在最终失败时明确报错，不返回空成功。抓取结果按响应编码解码，并在调用 AI 前抽取主体内容、呼号附近内容和图片文件名线索。QRZ.COM 密码、QRZ.CN 密码与 QRZ.CN Cookie 仅写入 Halo `Secret`，不写入 `SystemSetting.spec`，不进入导入导出数据包。
